...
public:
    ...
    uint32_t userid;
    const PyTuple* payload;
    std::optional <const PyDict*> named_payload;

    // this one is not used on client<->server traffic
    // is used as extra context for node<->node traffic
    std::optional <const PyDataType*> contextKey;

    PyDataType* operator= (const EVEPacket& oth);
};
