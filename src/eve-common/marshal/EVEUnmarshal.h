 ...
 */
extern PyDataType* Unmarshal (const Buffer& data);
/**
 ...
 * @return Ownership of Python object.
*/
extern PyDataType* InflateUnmarshal (const Buffer& data);
...
private:
    ...
    PyDataType* LoadNone () { return PyStatic.NewNone (); }

    /** Loads true boolean from stream. */
    PyDataType* LoadBoolTrue () { return PyStatic.NewTrue (); }
    /** Loads false boolean from stream. */
    PyDataType* LoadBoolFalse () { return PyStatic.NewFalse (); }

    /** Loads long long integer from stream. */
    PyDataType* LoadIntegerLongLong () { return new PyInt (Read <int64> ()); }
    /** Loads long integer from stream. */
    PyDataType* LoadIntegerLong () { return new PyInt (Read <int32> ()); }
    /** Loads signed short from stream. */
    PyDataType* LoadIntegerSignedShort () { return new PyInt (Read <int16> ()); }
    /** Loads byte integer from stream. */
    PyDataType* LoadIntegerByte () { return new PyInt (Read <int8> ()); }
    /** Loads variable length integer from stream. */
    PyDataType* LoadIntegerVar ();
    /** Loads minus one integer from stream. */
    PyDataType* LoadIntegerMinusOne () { return new PyInt (-1); }
    /** Loads zero integer from stream. */
    PyDataType* LoadIntegerZero () { return new PyInt (0); }
    /** Loads one integer from stream. */
    PyDataType* LoadIntegerOne () { return new PyInt (1); }

    /** Loads real from stream. */
    PyDataType* LoadReal () { return new PyFloat (Read <double> ()); }
    /** Loads zero real from stream. */
    PyDataType* LoadRealZero () { return new PyFloat (0.0); }

    /** Loads empty string from stream. */
    PyDataType* LoadStringEmpty () { return new PyString (""); }
    ...
    PyDataType* LoadWStringEmpty () { return new PyString (std::string (""), true); }
    ...
    PyDataType* LoadTupleEmpty () { return new PyTuple (0); }
    ...
    PyDataType* LoadListEmpty () { return new PyList (0); }
