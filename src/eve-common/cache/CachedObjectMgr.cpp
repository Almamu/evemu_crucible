 ...
 *
    ...
    PyTuple* first = new PyTuple(2);
        first->SetItem(0, new PyInt(Win32TimeNow()));
...
std::string CachedObjectMgr::OIDToString (PyDataType *objectID)
{
...
bool CachedObjectMgr::HaveCached (PyDataType *objectID) const
{
...
void CachedObjectMgr::InvalidateCache (PyDataType *objectID)
{
    const std::string str = OIDToString (objectID);
...
void CachedObjectMgr::UpdateCache (PyDataType *objectID, PyDataType **in_cached_data)
{
...
void CachedObjectMgr::_UpdateCache (PyDataType *objectID, PyBuffer **pbuf)
{
...
PyObject *CachedObjectMgr::MakeCacheHint (PyDataType *objectID)
{
    const std::string str = OIDToString (objectID);
...
PyObject *CachedObjectMgr::GetCachedObject (const std::string &objectID, PythonArena* arena)
{
    //this is sub-optimal, but it keeps things more consistent (in case StringCollapseVisitor ever gets more complicated)
    PyString* str = arena->String (objectID);
    PyObject* obj = GetCachedObject (str, arena);
    PyDecRef (str);
    return obj;
}

PyObject *CachedObjectMgr::GetCachedObject (PyDataType *objectID, PythonArena* arena)
{
    const std::string str = OIDToString (objectID);
    ...
    EVECachedObject cachedObject {
        .timestamp = res->second->timestamp,
        .version = res->second->version,
        .nodeID = HackCacheNodeID, // hack, doesn't matter until we have multi-node networks
        .shared = true,
        .cache = res->second->cache->clone (arena),
        .compressed = res->second->cache->size() > 0 && res->second->cache->content() [0] != MarshalHeaderByte,
        .objectID = res->second->objectID->clone (arena),
    };

    sLog.Debug ("CachedObjMgr","Returning cached object '%s' with checksum 0x%x", str.c_str (), cachedObject.version);

    return cachedObject.encode (arena);
}

bool CachedObjectMgr::IsCacheUpToDate (PyDataType *objectID, uint32 version, int64 timestamp)
{
    const std::string str = OIDToString (objectID);
 ...
 */
bool CachedObjectMgr::LoadCachedFromFile (const std::string &cacheDir, PyDataType *objectID)
{
    const std::string str = OIDToString (objectID);
...
bool CachedObjectMgr::SaveCachedToFile (const std::string &cacheDir, PyDataType *objectID) const
{
    const std::string str = OIDToString (objectID);
...
EVECachedObject::~EVECachedObject () {
    PySafeDecRef (cache);
    PySafeDecRef (objectID);
}
...
void EVECachedObject::dump (FILE *into, const char *pfx, bool contents_too)
{
    std::string s(pfx);
    s += "    ";
    fprintf(into, "%sCached Object:\n", pfx);
...
PyObject* EVECachedObject::encode(PythonArena* arena)
{
    // TODO: REVIEW THESE CLONE HERE
    return arena->Object (
        arena->String ("objectCaching.CachedObject"),
        arena->Tuple ({
            arena->Tuple ({arena->Int (timestamp), arena->Int (version)}),
            arena->None(),
            arena->Int (nodeID),
            arena->Int (shared ? 1 : 0),
            cache->clone(arena),
            arena->Int (compressed ? 1 : 0),
            objectID->clone(arena)
        })
    );
}
