...
public:
    ...
    static std::string OIDToString (PyDataType *objectID);

    bool HaveCached (const std::string &objectID) const;
    bool HaveCached (PyDataType *objectID) const;

    bool IsCacheUpToDate (PyDataType *objectID, uint32 version, int64 timestamp);
    ...
    void UpdateCache (PyDataType *objectID, PyDataType **in_cached_data);

    PyObject *MakeCacheHint (PyDataType *objectID);
    PyObject *MakeCacheHint (const std::string &objectID);

    PyObject *GetCachedObject (PyDataType *objectID, PythonArena* arena);
    PyObject *GetCachedObject (const std::string &objectID, PythonArena* arena);
...
//OLD CCP FILE BASED ACCESS:
    ...
    bool LoadCachedFromFile (const std::string &cacheDir, PyDataType *objectID);
    bool SaveCachedToFile (const std::string &cacheDir, const std::string &objectID) const;
    bool SaveCachedToFile (const std::string &cacheDir, PyDataType *objectID) const;
...
protected:
    ...
    void _UpdateCache (PyDataType *objectID, PyBuffer **buffer);
    ...
    public:
        ...
        PyDataType *objectID;    //we own this
        int64 timestamp;
        uint32 version;
...
public:
    ...
    PyObject *encode (PythonArena* arena);
