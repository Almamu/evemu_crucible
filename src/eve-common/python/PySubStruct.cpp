...
PySubStruct::PySubStruct (PyDataType* content, bool checkArenaOwnership) :
    PyDataType (Type),
    mContent (content) {
...
PySubStruct* PySubStruct::clone (PythonArena* arena) const {
    return arena->SubStruct (this->content()->clone(arena));
}
