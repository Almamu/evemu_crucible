...
bool PyDumpVisitor::isFullNested() const {
    return mIsFullNested;
}
...
bool PyDumpVisitor::VisitObjectEx (const PyObjectEx* rep) {
    _print ("%s[PyObjectEx %s]", _pfx(), rep->isType2 () ? "Type2" : "Type1");
...
bool PyDumpVisitor::VisitPackedRow (const PyPackedRow* rep) {
    ...
    for (uint32_t i = 0; i < rep->header ()->count (); i ++) {
        _print ("%s[PyPackedRowColumn '%s' (%s)]", _pfx (), rep->header ()->name (i)->content().c_str(), DBTYPE_GetName(rep->header ()->type (i)));
