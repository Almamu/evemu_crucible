...
CRowset::CRowset (DBRowDescriptor* header, bool checkArenaOwnership) :
...
PyPackedRow* CRowset::insert () {
    PyPackedRow* row = this->arena()->PackedRow (mHeader);
 ...
 */
PyPackedRow* CRowset::insert (std::initializer_list <PyDataType*> values, bool checkArenaOwnership) {
    PyPackedRow* row = this->arena()->PackedRow (mHeader, values, checkArenaOwnership);

    this->list ()->add (row, checkArenaOwnership);
...
PyTuple* CRowset::_CreateArgs()
{
    return this->arena()->Tuple ({
        this->arena()->Token ("dbutil.CRowset")
    });
}
