...
#ifndef __CINDEXEDROWSET_H__
#define __CINDEXEDROWSET_H__
...
public:
     ...
     */
    CIndexedRowset (DBRowDescriptor* header, const char* indexColumn);

    /**
     * @return Adds an empty row that can be filled manually
     */
    PyPackedRow* insert (PyDataType* key);
    /**
     * @return Adds a new row with the given values
     */
    PyPackedRow* insert (PyDataType* key, std::initializer_list <PyDataType*> values);

    /**
     * @param index The row to get
     * @return The requested row
     */
    PyPackedRow* get (PyDataType* index) const;
...
private:
    ...
    static PyDict* _CreateKeywords(DBRowDescriptor* header, const char* indexColumn);
};


#endif /* !__CINDEXEDROWSET_H__ */
